...
export class ProductsService {
  constructor(
    ...
    @InjectRepository(FragranceNote)
    private readonly noteRepo: Repository<FragranceNote>,
    @InjectRepository(ProductIngredient)
    private readonly ingredientRepo: Repository<ProductIngredient>,
  ...
  async findAll(
    ...
    if (query.concentration) {
      qb.andWhere('product.concentration = :concentration', {
    ...
    if (query.occasion) {
      qb.andWhere(':occasion = ANY(product.occasion)', {
  ...
  async findBySlug(
    ...
    Product & { related: Product[]; frequentlyBoughtTogether: Product[] }
  > {
  ...
  private async findFrequentlyBoughtTogether(
    ...
    const coPurchased = await this.orderItemRepo
      ...
      .innerJoin(
        'order_items',
        'oi2',
        'oi2.order_id = oi1.order_id AND oi2.variant_id != oi1.variant_id',
      )
  ...
  async create(dto: CreateProductDto): Promise<Product> {
    const product = this.productRepo.create({
      ...
      fragranceNotes:
        dto.fragranceNotes?.map((n) => this.noteRepo.create(n)) ?? [],
      ingredients:
        dto.ingredients?.map((name) => this.ingredientRepo.create({ name })) ??
        [],
    });
  ...
  async update(id: string, dto: UpdateProductDto): Promise<Product> {
    ...
    const { variants, images, fragranceNotes, ingredients, ...scalarFields } =
      dto;
    ...
    if (fragranceNotes)
      product.fragranceNotes = fragranceNotes.map((n) =>
        this.noteRepo.create(n),
      );
    if (ingredients)
      product.ingredients = ingredients.map((name) =>
